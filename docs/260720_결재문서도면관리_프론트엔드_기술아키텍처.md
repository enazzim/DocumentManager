...
## 5. 사용자 인증 및 JWT/권한 라우팅 통제 아키텍처 (Auth & Route Guard)

### 5.1. AuthStore 상태 및 JWT Refresh Flow (Zustand)
`useAuthStore` 스토어를 통해 로그인한 사용자 프로필(`user`), AccessToken 메메모리 보관, 로그인/로그아웃 및 401 토큰 만료 시 비동기 RefreshToken 자동 재발급을 통제합니다.

```mermaid
sequenceDiagram
    autonumber
    participant UI as React Component / ProtectedRoute
    participant Axios as Axios Interceptor
    participant Store as Zustand AuthStore
    participant API as Spring Boot (dms-backend)

    UI->>Axios: API 요청 (Authorization: Bearer AccessToken)
    Axios->>API: HTTP Request
    API-->>Axios: 401 Unauthorized (AccessToken 만료)
    Axios->>Store: refreshAccessToken() 액션 수행
    Store->>API: POST /api/v1/auth/refresh (RefreshToken)
    alt 토큰 재발급 성공
        API-->>Store: 200 OK (새 AccessToken 반환)
        Store-->>Axios: 새 AccessToken 메모리 갱신
        Axios->>API: 기존 원본 요청 재시도 (Retry Original Request)
        API-->>UI: 정상 응답 데이터 반환
    else 토큰 재발급 실패 / 만료
        API-->>Store: 403 / 401 (RefreshToken 만료)
        Store->>Store: logout() 호출 (인증 상태 세션 초기화)
        Store-->>UI: /login 페이지로 강제 라우팅
    end
```

### 5.2. ProtectedRoute 및 RBAC / ABAC 권한 통제 컴포넌트
보호된 라우트 진입 시 사용자의 로그인 여부와 요구되는 RBAC 역할(`requiredRoles`)을 검증하는 Higher-Order Guard 컴포넌트입니다.
...
### 5.3. 개발 및 운영 환경 CORS 차단 방어 체계 (CORS & Proxy Strategy)
브라우저 Same-Origin Policy에 따른 CORS (Cross-Origin Resource Sharing) 에러를 원천 차단하기 위해 이중 방어 전략을 취합니다.

- **백엔드 (Spring Boot CorsConfigurationSource):** `CorsConfig` 클래스를 통해 허용 Origin (`http://localhost:5173`), HTTP 메서드(`GET/POST/PUT/DELETE/OPTIONS`), 클라이언트 응답 노출 헤더(`Authorization`)를 전역 승인.
- **프론트엔드 (Vite Development Server Proxy):** `vite.config.ts` 내 `server.proxy` 설정을 적용하여 개발 중 `/api` 요청을 동일 출처인 것처럼 `http://localhost:8080`으로 투명하게 포워딩.

---

## 6. OpenAPI-to-Zod 빌드 파이프라인 자동화 (Code Generation)
백엔드 DDL의 필드 크기나 제약 조건 변경이 프론트의 Zod 폼 유효성 스키마와 불일치하여 발생하는 API 기각 오류를 Vite 빌드 스크립트 기반 `openapi-to-zod` 파이프라인으로 완전 자동화합니다.

---

## 7. 마치며 및 결론
본 프론트엔드 기술 아키텍처 명세서(v1.2)에서는 **window.onbeforeprint의 한계를 Keydown Key Intercept 우회 인쇄 기법으로 돌파**하고, **음영 지역에서의 오프라인 한계를 전체 ACTIVE 도면 메타데이터 백그라운드 프리패치(Pre-fetch) IndexedDB 이식**, 그리고 **Zustand 기반 JWT 자동 갱신 Interceptor 및 ProtectedRoute 라우트 보안 통제 아키텍처**를 완비하였습니다.
