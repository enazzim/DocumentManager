...
CREATE TABLE users (
    ...
    department_code VARCHAR(50) NOT NULL,
    auth_provider VARCHAR(20) NOT NULL DEFAULT 'LOCAL', -- LOCAL, SSO_OIDC, SSO_SAML (하이브리드 인증)
    provider_id VARCHAR(100), -- 외부 SSO 식별키
...
### 4.3. 백엔드 전역 공통 모듈 및 예외/스토리지 인프라 명세 (Global Common Infrastructure)

**1. 로컬 스토리지 Fallback 모듈 (`LocalStorageService`)**
  - **설계 목적:** AWS S3 자격증명이 부재한 로컬 개발 PC 환경에서도 도면 파일 업로드/다운로드를 즉시 구동 테스트할 수 있도록 `StorageService` 인터페이스 아래 로컬 디렉토리(`uploads/`) 저장소를 구현하고, 프로필 설정(`storage.type: local`)으로 유연하게 스토리지 전환을 지원합니다.

**2. 비동기 작업 및 SSE Emitter 커넥션 관리자 (`AsyncConfig` & `SseEmitterRepository`)**
  - **설계 목적:** 바이어 도면 비동기 무결성 검사(`@Async("fileValidationTaskExecutor")`) 및 태블릿 단말기 원격 강제 리로드 명령 송출을 위한 `SseEmitterRepository`를 전역 싱글톤으로 구축하여 커넥션 고갈 및 스레드 병목을 방지합니다.

**3. 1회용 보안 토큰 QR 코드 생성 유틸리티 (`QrCodeGenerator`)**
  - **설계 목적:** `com.google.zxing:core` 및 `javase` 기반으로 도면 출력물 스캔 검증(`/verify`)을 위한 HMAC 서명 토큰 및 QR 코드 이미지를 PNG/Base64 스트림으로 즉시 생성하는 유틸리티 모듈을 배치합니다.

**4. 전역 공통 응답 및 예외 처리기 (`ApiResponse<T>` & `GlobalExceptionHandler`)**
  - **설계 목적:** 모든 REST API 응답 결과를 `ApiResponse<T>` 포맷으로 통일하고, 비즈니스 에러(`CustomException`), 입력 검증 에러(`MethodArgumentNotValidException`), 401/403/404/500 에러를 `@RestControllerAdvice`로 캐치하여 정형화된 JSON 형태의 `ErrorCode`로 반환합니다.
...
**9. 관리자 전용 사용자 생성 및 권한/역할 매핑**
  - **Method & Path:** `POST /api/v1/admin/users` 및 `PUT /api/v1/admin/users/{id}/roles`
  - **기능 및 역할:** 관리자가 신규 사용자를 직접 발급/등록하고, 사용자의 부서, ABAC 보안등급, ABAC 담당기종 및 RBAC 역할(`ROLE_USER`, `ROLE_APPROVER`, `ROLE_ADMIN`) 매핑을 수정 갱신합니다.
...
| **Admin Dashboard**| 사용자 및 역할/권한 관제 | `GET /api/v1/admin/users` | `useQuery` (Admin 전용) |
| **Admin Dashboard**| 신규 사용자 직접 등록 발급 | `POST /api/v1/admin/users` | `useMutation` |
| **Admin Dashboard**| 역할(Role) & ABAC 속성 매핑 | `PUT /api/v1/admin/users/{id}/roles` | `useMutation` |
